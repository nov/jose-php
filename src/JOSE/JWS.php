...
class JOSE_JWS extends JOSE_JWT {
    ...
    private function rsa($public_or_private_key, $padding_mode) {
        ...
        } else if ($public_or_private_key instanceof phpseclib\Crypt\RSA) {
    ...
    private function _sign($private_key_or_secret) {
        ...
        switch ($this->header['alg']) {
            ...
            case 'RS384':
            case 'RS512':
                return $this->rsa($private_key_or_secret, phpseclib\CRYPT\RSA::SIGNATURE_PKCS1)->sign($signature_base_string);
            case 'ES256':
            case 'ES384':
            case 'ES512':
            ...
            case 'PS384':
            case 'PS512':
                return $this->rsa($private_key_or_secret, phpseclib\CRYPT\RSA::SIGNATURE_PSS)->sign($signature_base_string);
            default:
                throw new JOSE_Exception_UnexpectedAlgorithm('Unknown algorithm');
        }
    }
    ...
    private function _verify($public_key_or_secret, $expected_alg = null) {
        ...
        switch ($expected_alg) {
            ...
            case 'RS384':
            case 'RS512':
                return $this->rsa($public_key_or_secret, phpseclib\CRYPT\RSA::SIGNATURE_PKCS1)->verify($signature_base_string, $this->signature);
            case 'ES256':
            case 'ES384':
            case 'ES512':
                throw new JOSE_Exception_UnexpectedAlgorithm('Algorithm not supported');
            ...
            case 'PS384':
            case 'PS512':
                return $this->rsa($public_key_or_secret, phpseclib\CRYPT\RSA::SIGNATURE_PSS)->verify($signature_base_string, $this->signature);
            default:
                throw new JOSE_Exception_UnexpectedAlgorithm('Unknown algorithm');
        }
    }
